...
const Boutique = () => {
  ...
  const buyItem = async (item) => {
    ...
    if (userData.coins < item.price) {
      alert("Not enough coins, bestie! 🥺 Time to complete more tasks.");
      return;
    }
    
    setBuying(item.id);
    try {
      await updateProfile({ coins: userData.coins - item.price });
      alert(`You bought ${item.name}! ✨`);
    } catch (e) {
      alert("Failed to buy. Are you connected to Firebase?");
    }
    setBuying(null);
  };

  if (!userData) return <div className="empty-state"><div className="empty-title">Loading Boutique...</div></div>;
  ...
  return (
    <div>
      <div className="shop-top">
        ...
        <div className="coins">🪙 {userData.coins.toLocaleString()} <span style={{ fontFamily: "'Gaegu', cursive", color: 'var(--text-light)', fontSize: '0.85rem' }}>coins</span></div>
      </div>
          ...
          <div key={item.id} className="shop-item">
            ...
            <div className="shop-info">
              ...
              <button 
                ...
                style={{ opacity: userData.coins < item.price ? 0.7 : 1 }}
              >
                {buying === item.id ? 'Buying...' : `🪙 ${item.price} — Buy Now`}
              </button>
            </div>
          </div>
        ))}
